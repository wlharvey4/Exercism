...
#+Author:Pinecone062
#+Date:Last updated 2020-11-13 10:25
#+Macro: version Version 0.5.25
{{{title}}} {{{version}}} {{{date}}}
* Exercism Home
:PROPERTIES:
:unnumbered: t
...
#+begin_src lisp
  ...
  (defun ea-roman-numeral (n)
    (loop for (r d) in (reverse roman-numerals)
          with roman = "I"
          do (format t "Roman numeral: ~a  Digit: ~a~%" r d)
          finally (return roman)))

...
#+Author:Pinecone062
#+Date:Last updated 2020-11-06 09:52
#+Macro: version Version 0.5.17
{{{title}}} {{{version}}} {{{date}}}
* Exercism Home
:PROPERTIES:
:unnumbered: t
...
#+begin_src lisp
  ...
  (defun anagrams-for (subject candidates)
    ...
    (let ((subject-sorted (sort (string-downcase subject) #'char<)))
      (remove-if-not
       (lambda (candidate)
         (unless (string= (string-downcase subject) (string-downcase candidate))
           (let ((candiate-sorted (sort (string-downcase candidate) #'char<)))
             (string= subject-sorted candiate-sorted))))
         candidates)))
#+end_src

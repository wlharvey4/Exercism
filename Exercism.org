...
#+Author:Pinecone062
#+Date:Last updated 2020-11-15 07:30
#+Macro: version Version 0.5.27
{{{title}}} {{{version}}} {{{date}}}
* Exercism Home
:PROPERTIES:
:unnumbered: t
...
#+begin_src lisp
  ...
  (defun simplify (r pos ds)
    ;(format t "simplifying: pos: ~a   ds:~a   r: ~a" pos ds r)
    (let ((r2 (if (eql (elt ds (1+ pos)) 1)
                  (car (elt roman-numerals (+ 2 pos)))
                  (car (elt roman-numerals (+ 1 pos))))))
      (format nil "~a~a" r2 r)))


  ;; (defun m
  (defun mult (d pos ds)
    ;(format t "mult--d: ~a  pos: ~a  ds: ~a  " d pos ds)
    (if (or (< pos 0) (> pos 6))
        ""
        (let ((r (car (elt roman-numerals pos)))
              (a ""))
          ;(format t "r: ~a~%" r)
          (if
           (and (< pos 5) (= d 4))
  ...
  (defun d2r (ds)
    "Transform a list of digits representing Roman numerals into a
  string of Roman numerals.  Then simplify and return.

  Simplify means to transform 'IIII' => 'IV', 'VIIII' => 'IX', etc."
    ;(format t "d2r--ds: ~a~%" ds)
    (loop for d in ds with roman = "" with pos = 0
          do
             ;; mult produces a repeated Roman numeral (d times) of
             ;; magnitude pos, where pos is the placeholder 0<=pos<=6
             ;; representing the Roman numeral
             (setf roman (concatenate 'string roman (mult d pos ds)))
             (when (and (eql d 4) (< pos 5)) (setf (elt ds (1+ pos)) 0))
             (incf pos)
  ...
  digits, and simplify."
    ...
    (loop for (nil val) in (reverse roman-numerals) ; only need the
                                                    ...
                                                    ; highest value and
                                                    ; work down.
          with roman = () ; will hold the result of the loop; will be a
          ...
          do
             ...
             ;; iteration
             (setf roman (cons (truncate (/   num val)) roman)
                   num                   (mod num val))
          finally (return (d2r roman)))) ; transform the list of digits
                                         ; into Roman digits
#+end_src

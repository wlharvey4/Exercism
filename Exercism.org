...
#+Author:Pinecone062
#+Date:Last updated 2020-11-24 23:56
#+Macro: version Version 0.5.32
{{{title}}} {{{version}}} {{{date}}}
* Exercism Home
:PROPERTIES:
:unnumbered: t
...
#+begin_src lisp :tangle common-lisp/word-count/word-count.lisp :mkdirp yes
  ...
  (defun count-words (sentence)
    (setq set ())
    (dolist (w (split "[^\\w']" (string-upcase sentence)) set)
      (let ((e (assoc w set #'string=)))
        (if (consp e)
            (progn
              (format t "Adding element ~a~%" e)
              (incf (cdr e)))
            (cons (cons w 0) set)))))

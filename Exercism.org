...
#+Author:Pinecone062
#+Date:Last updated 2019-06-05 10:17
#+Macro: version Version 0.4.6
{{{title}}} {{{version}}} {{{date}}}
* Exercism Home
:PROPERTIES:
:unnumbered: t
...
#+begin_src markdown :tangle python/matrix/README.md
# Matrix
...
#+begin_src python :tangle python/matrix/matrix.py :mkdirp yes
  class Matrix(object):
      def __init__(self, matrix_string):
          ...
          for row in rows:
              col = []
              col_as_str = row.split(' ')
              for num in col_as_str:
                  col.append(int(num))

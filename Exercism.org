...
#+Author:Pinecone062
#+Date:Last updated 2020-11-25 07:40
#+Macro: version Version 0.5.38
{{{title}}} {{{version}}} {{{date}}}
* Exercism Home
:PROPERTIES:
:unnumbered: t
...
#+begin_src lisp :tangle common-lisp/word-count/word-count.lisp :mkdirp yes
  ...
  (defpackage #:word-count
    (:use #:cl)
    (:import-from #:cl-ppcre #:split #:regex-replace #:create-scanner)
    (:export #:count-words))
  (in-package #:word-count)

  (defun count-words (sentence)
    (let ((set ())
          (scnr (create-scanner "[^\\w']+" :single-line-mode t)))
      (dolist (w (split scnr
                        (string-downcase

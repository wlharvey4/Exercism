...
#+Author:Pinecone062
#+Date:Last updated 2021-01-27 23:56
#+Macro: version Version 0.5.48
{{{title}}} {{{version}}} {{{date}}}
* Exercism Home
:PROPERTIES:
:unnumbered: t
...
For example, 1997 is not a leap year, but 1996 is. 1900 is not a leap year, but
2000 is.
*** Emacs Lisp Leap Scripts
**** Emacs Lisp Leap Download Script
#+begin_src shell :exports never
  exercism download --exercise=leap --track=elisp
#+end_src

#+RESULTS:
: /usr/local/dev/programming/Exercism/emacs-lisp/leap

**** Emacs Lisp Leap Submit Script
#+begin_src shell :exports never
  exercism submit elisp/leap/leap.el
#+end_src
*** Emacs Lisp Leap Meta Files
#+caption: Emacs Lisp Leap README
#+name:emacs-lisp-leap-readme
#+header: :tangle elisp/leap/README.md
#+header: :mkdirp yes
#+begin_src md
...
#+name:emacs-lisp-leap-metadata
#+header: :tangle elisp/leap/.exercism/metadata.json
#+header: :mkdirp yes
#+begin_src js
  {
      "track":"emacs-lisp",
      "exercise":"leap",
      "id":"73cc21532b454254bac457561f7c555c",
      "url":"https://exercism.io/my/solutions/73cc21532b454254bac457561f7c555c",
      "handle":"wlharvey4",
      "is_requester":true,
      "auto_approve":false
  }
...
#+RESULTS: emacs-lisp-leap-metadata

*** Emacs Lisp Leap Code
#+caption: Emacs Lisp Leap Code
#+name: emacs-lisp-leap-code
#+header: :tangle elisp/leap/leap.el
#+header: :mkdirp yes
#+begin_src emacs-lisp
...
#+CAPTION: Emacs Lisp Leap Run Tests
#+NAME: emacs-lisp-leap-run-tests
#+HEADER: :results output
#+HEADER: :exports both
#+HEADER: :dir elisp/leap
#+begin_src shell
  emacs -batch -l ert -l leap-test.el -f ert-run-tests-batch-and-exit
#+end_src

#+RESULTS: emacs-lisp-leap-run-tests

*** Emacs Lisp Leap Tests
#+caption: Emacs Lisp Leap Tests
#+name: emacs-lisp-leap-tests
#+header: :tangle elisp/leap/leap-test.el
#+header: :mkdirp yes
#+begin_src emacs-lisp
...
#+header: :mkdirp yes :tangle common-lisp/twelve-days/.exercism/metadata.json
#+begin_src js
  {
      "track":"common-lisp",
...
#+RESULTS:
: https://exercism.io/my/solutions/76d50a3a72f4491cb24d98b5f5e36b7f
...
#+begin_src lisp
  ...
  (defconstant +BEGINNING+ "On the ~:R day of Christmas my true love gave to me: ~A")
  ...
  (defun beginning-day (day)
    "Given a number for day representing the first day of the song,
  return the string."
    (format nil +BEGINNING+ day (gift day)))

  (defun one-iteration (start-day)
    "Given a number representing a start day, run through all of the
  iterations down to one."
    (let ((song (beginning-day start-day)))
       (do ((cur-day (1- start-day) (1- cur-day))
            (_and (if (= start-day 2) "and " "") (if (= cur-day 2) "and " "")))
          ((zerop cur-day) song)
        (setf song (format nil "~A~A~A" song _and (gift cur-day))))))

  (defun recite (&optional begin end)
    "Returns a string of the requested verses for the 12 Days of
  Christmas."
    (let ((end (if (and (null begin) (null end)) 12
                   (if (null end) begin end)))
          (begin (if (null begin) 1 begin))
          (full-song ""))
      (do ((cur-day begin (1+ cur-day)))
          ((> cur-day end) full-song)
        (setf full-song (format nil "~A~&~A" full-song (one-iteration cur-day))))))
#+end_src
...
TWELFTH_DAY_TWELVE_DRUMMERS_DRUMMING: 1 assertions passed, 0 failed.

SEVENTH_DAY_SEVEN_SWANS_A_SWIMMING: 1 assertions passed, 0 failed.

RECITES_FIRST_THREE_VERSES_OF_THE_SONG: 1 assertions passed, 0 failed.

THIRD_DAY_THREE_FRENCH_HENS: 1 assertions passed, 0 failed.

FOURTH_DAY_FOUR_CALLING_BIRDS: 1 assertions passed, 0 failed.

SIXTH_DAY_SIX_GEESE_A_LAYING: 1 assertions passed, 0 failed.

FIRST_DAY_A_PARTRIDGE_IN_A_PEAR_TREE: 1 assertions passed, 0 failed.

RECITES_THE_WHOLE_SONG: 2 assertions passed, 0 failed.

TENTH_DAY_TEN_LORDS_A_LEAPING: 1 assertions passed, 0 failed.

RECITES_THREE_VERSES_FROM_THE_MIDDLE_OF_THE_SONG: 1 assertions passed, 0 failed.

FIFTH_DAY_FIVE_GOLD_RINGS: 1 assertions passed, 0 failed.

ELEVENTH_DAY_ELEVEN_PIPERS_PIPING: 1 assertions passed, 0 failed.

NINTH_DAY_NINE_LADIES_DANCING: 1 assertions passed, 0 failed.

EIGHTH_DAY_EIGHT_MAIDS_A_MILKING: 1 assertions passed, 0 failed.

SECOND_DAY_TWO_TURTLE_DOVES: 1 assertions passed, 0 failed.

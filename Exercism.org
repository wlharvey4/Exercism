...
#+Author:Pinecone062
#+Date:Last updated 2020-11-14 07:44
#+Macro: version Version 0.5.26
{{{title}}} {{{version}}} {{{date}}}
* Exercism Home
:PROPERTIES:
:unnumbered: t
...
#+begin_src lisp
  ...
  (defun simplify (roman)
    roman)

  (defun mult (ds pos)
    (if (or (< pos 0) (> pos 6))
         ""
        (let* ((rd (elt roman-numerals pos))
               (r (car rd))
               (a ""))
          (dotimes (it ds a)
  ...
  Simplify means to transform 'IIII' => 'IV', 'VIIII' => 'IX', etc."
    (loop for d in ds with roman = "" with pos   = 0
          do
             ...
             (setf roman (concatenate 'string roman (mult d pos)))
             (incf pos)
             ;; simplify turns IIII into IV, and XXXX into XL etc.
          finally (return (simplify (reverse roman)))))

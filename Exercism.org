...
#+Author:Pinecone062
#+Date:Last updated 2019-05-30 13:03
#+Macro: version Version 0.3.6
{{{title}}} {{{version}}} {{{date}}}
* Exercism Home
:PROPERTIES:
:unnumbered: t
...
(defun response-for (response)
  ...
  (cond ((string-match "^[[:upper:]\s]+\\?$" response) "Calm down, I know what I'm doing!")
        ((string-match "\\`[^\n]+\\?\s*\\'" response) "Sure.")
        ((string-match "\\`[[:space:]]*\\'" response) "Fine. Be that way!")
        ((string-match ".[[:upper:]].*!$\\|\\`[^[:lower:][:digit:]]+\\'" response) "Whoa, chill out!")
        (t "Whatever.")))
